...
WORKDIR /ai300-capstone-app

# Copy the requirements.txt file and required directories into docker image
COPY ./requirements.txt /ai300-capstone-app/requirements.txt
COPY ./src /ai300-capstone-app/src
COPY ./model /ai300-capstone-app/model
...
ENV PYTHONPATH /ai300-capstone-app/src

# Install python package dependancies, without saving downloaded packages locally
RUN pip install -r /ai300-capstone-app/requirements.txt --no-cache-dir
